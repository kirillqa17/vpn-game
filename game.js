...
async function checkAndDecrementAttempts() {
    try {
        ...
        // Проверяем попытки
        const attemptsResponse = await fetch(`https://game.svoivpn.duckdns.org/attempts/${userId}`);
        ...
        // Списываем попытку
        const updateResponse = await fetch(`https://game.svoivpn.duckdns.org/attempts/${userId}`, {
            method: 'POST',
...
function sendGameResult(score) {
    fetch(`https://game.svoivpn.duckdns.org/points/${userId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function decrementAttempts() {
    try {
        const response = await fetch(`https://game.svoivpn.duckdns.org/attempts/${userId}`);
...
async function updateUserRecord(newScore) {
    let record;
    try {
        const response = await fetch(`https://game.svoivpn.duckdns.org/record/${userId}`);
        const data = await response.json();
