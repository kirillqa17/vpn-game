...
async function loadUserData() {
    try {
        // Загружаем очки
        const pointsResponse = await fetch(`https://svoivpn.duckdns.org:4443/points/${window.userId}`);
        ...
        // Загружаем попытки
        const attemptsResponse = await fetch(`https://svoivpn.duckdns.org:4443/attempts/${window.userId}`);
...
async function checkClaimStatus() {
    try {
        const response = await fetch(`https://svoivpn.duckdns.org:4443/claim/${window.userId}`);
...
async function handleClaimClick() {
    ...
    try {
        ...
        if (data.next_claim_time) {
            // Обновляем попытки
            const attemptsResponse = await fetch(`https://svoivpn.duckdns.org:4443/attempts/${window.userId}/add`, {
                method: 'POST',
...
async function updatePlayButtonState() {
    try {
        const response = await fetch(`https://svoivpn.duckdns.org:4443/attempts/${window.userId}`);
        const data = await response.json();
...
async function loadUserRecord() {
    try {
        const response = await fetch(`https://svoivpn.duckdns.org:4443/record/${window.userId}`);
        const data = await response.json();
